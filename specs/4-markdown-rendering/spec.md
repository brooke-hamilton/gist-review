...
3. **Given** a code block specifies a language, **When** the content is displayed, **Then** the code is visually formatted appropriately for that language.
...
- What happens when Markdown contains HTML tables?
  - Tables should render but be sanitized to remove any dangerous attributes.
- What happens when Markdown contains images with external URLs?
  - Images should render but potentially be flagged as external content in future iterations.
...
- **FR-003**: System MUST render Markdown links as clickable HTML anchors
...
- Syntax highlighting for code blocks is a nice-to-have; basic monospace styling is sufficient
- The CommonMark specification is the baseline for Markdown parsing behavior
